...
if (!class_exists('LatePoint')) :
	...
	final class LatePoint {
		...
		public function define_constants() {
			...
			$this->define('LATEPOINT_BLOCKS_BUILD_URL', $this->blocks_build_url());
			$this->define('LATEPOINT_ABSPATH', dirname(__FILE__) . '/');
			$this->define('LATEPOINT_LIB_ABSPATH', LATEPOINT_ABSPATH . 'lib/');
			$this->define('LATEPOINT_CONFIG_ABSPATH', LATEPOINT_LIB_ABSPATH . 'config/');
			$this->define('LATEPOINT_BLOCKS_ABSPATH', LATEPOINT_ABSPATH . 'blocks/build/');
			...
			$this->define('LATEPOINT_WP_ADMIN_ROLE', 'administrator');
			$this->define('LATEPOINT_WP_AGENT_ROLE', 'latepoint_agent');
			$this->define('LATEPOINT_WP_MANAGER_ROLE', 'latepoint_manager');

			$this->define('LATEPOINT_USER_TYPE_ADMIN', 'admin');
			$this->define('LATEPOINT_USER_TYPE_AGENT', 'agent');
			$this->define('LATEPOINT_USER_TYPE_CUSTOM', 'custom');
			$this->define('LATEPOINT_USER_TYPE_CUSTOMER', 'customer');
			...
			$this->define('LATEPOINT_TABLE_PROCESS_JOBS', $wpdb->prefix . 'latepoint_process_jobs');
			...
			$this->define('LATEPOINT_JOB_STATUS_COMPLETED', 'completed');
			$this->define('LATEPOINT_JOB_STATUS_SCHEDULED', 'scheduled');
			$this->define('LATEPOINT_JOB_STATUS_CANCELLED', 'cancelled');
			$this->define('LATEPOINT_JOB_STATUS_ERROR', 'error');
			...
			$this->define('LATEPOINT_DATETIME_DB_FORMAT', 'Y-m-d H:i:s');
			...
			$this->define('LATEPOINT_VALUE_ON', 'on');
			$this->define('LATEPOINT_VALUE_OFF', 'off');

			$this->define('LATEPOINT_STATUS_ACTIVE', 'active');
			$this->define('LATEPOINT_STATUS_DISABLED', 'disabled');
			...
			if ($processor_code == 'wp_mail') {
				echo '<div class="sub-section-row">
					      <div class="sub-section-label">
					        <h3>'.__('Email Settings', 'latepoint').'</h3>
					      </div>
		...
		public function clear_old_activity_logs(){
			...
			$wpdb->query($wpdb->prepare("DELETE FROM {$activity->table_name} WHERE created_at < %s", $cutoff));
		}
		...
		public function customer_logout() {
			if (isset($_GET['os-action'])) {
				...
				nocache_headers();
				wp_redirect(OsSettingsHelper::get_customer_login_url(), 302);
				exit;
			}
		}
		...
		*/
        ...
        'included_phone_countries' => json_encode(OsSettingsHelper::get_included_phone_countries()),
				...
				'mask_phone_number_fields' => OsSettingsHelper::is_on('mask_phone_number_fields'),
				...
				'nulled_by' => 'r3turn_z3r0 on babia.to'
			];
			...
			wp_register_script('latepoint-main-front', $this->public_javascripts() . 'front.js', ['jquery', 'latepoint-vendor-front', 'wp-i18n'], $this->version);
		...
		public function load_admin_scripts_and_styles() {
			// Stylesheets
			wp_enqueue_style('latepoint-google-fonts', 'https://fonts.googleapis.com/css?family=Barlow:400,500,600&display=swap&subset=latin-ext', false);
			...
			wp_enqueue_script('latepoint-main-admin', $this->public_javascripts() . 'admin.js', ['jquery', 'latepoint-vendor-admin', 'wp-i18n'], $this->version);
        ...
        'included_phone_countries' => json_encode(OsSettingsHelper::get_included_phone_countries()),
			...
			OsBlockHelper::load_admin_scripts_and_styles($this->version);
...
$latepoint_session_salt = 'NmQ3OTBhMTItYjY5ZS00ZGFlLTg5ODMtNTlkYzVjZTM4MTVi';
